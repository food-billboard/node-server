const { verifySocketIoToken, otherToken, RoomModel, UserModel } = require("@src/utils")
...
const getDetail = socket => async (data) => {
  ...
  if(token) {
    ...
    .then(userId => {
      ...
      .exec()
      .then(data => {
        if(!data) return Promise.reject({ errMsg: 'error' })
        const { members } = data
        return {
          members: members.map(m => {
            const { type, content: { text, video, image, audio }, ...nextData } = m
            let newContent
            switch(type) {
              case "IMAGE": 
                newContent = image
