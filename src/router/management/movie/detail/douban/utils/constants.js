...
const MOVIE_NAME = async (page) => {
  const name = await page.$eval('#content h1 span', value => {
...
const MOVIE_POSTER = async (page) => {
  const image = await page.$eval(`${BASE_INFO_WRAPPER} #mainpic img`, value => {
    return value.src 
  })
  return image 
}
...
const MOVIE_CLASSIFY = async (page) => {
  const classify = await page.$$eval(`${BASE_INFO_WRAPPER} #info span[property='v:genre']`, value => {
    return value.map(item => {
...
const MOVIE_RATE = async (page) => {
  const rate = await page.$eval('#interest_sectl .rating_num', value => {
...
const MOVIE_DESCRIPTION = async (page) => {
  try {
    const allDescription = await page.$eval(`.related-info .all.hidden`, value => {
      return value ? value.innerHTML : ''
    })
    return allDescription.trim()
  }catch(err) {
    const hiddenDescription = await page.$eval(`.related-info span[property='v:summary']`, value => {
      return value.innerHTML
    })
