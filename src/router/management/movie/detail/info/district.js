...
const { DistrictModel, UserModel, dealErr, notFound, Params, responseDataDeal, MOVIE_SOURCE_TYPE, ROLES_MAP, verifyTokenToData, getWordPinYin } = require('@src/utils')
...
.delete('/', async(ctx) => {
  ...
  const data = await DistrictModel.deleteMany({
    _id: { $in: _ids }
  })
  .then(data => {
